...
for i in {1..1}
do
  ./bin/spark-submit --class org.apache.spark.examples.SparkPageRank --deploy-mode cluster --master mesos://114.212.83.104:7077 --conf spark.default.parallelism=8 --conf spark.executor.cores=1 --conf spark.mesos.coarse=true --conf spark.shuffle.service.enabled=true --conf spark.dynamicAllocation.enabled=true --conf spark.dynamicAllocation.executorIdleTimeout=60s --conf spark.dynamicAllocation.initialExecutors=1 --conf spark.dynamicAllocation.minExecutors=1 hdfs://114.212.83.104:9000/user/original-spark-examples_2.11-2.0.0.jar hdfs://114.212.83.104:9000/user/pagerank_10.txt 100
#  sleep 0.01
done
